...
def compute_metrics(n_season, data_to_cluster, idx):
    """
    Calcola i centroidi e l'errore per ciascuna stagione.

    Parameters:
    n_season (int): Numero di stagioni (cluster).
    data_to_cluster (array): Serie temporale da clusterizzare.
    idx (list of arrays): Lista degli indici per ciascun cluster.

    Returns:
    tuple: Lista di centroidi per ciascun cluster, lista di errori per ciascun cluster.
    """
    centroids = []
    error = []
...
def single_fit_optimized(data_to_cluster, n_seas, n_days):
    """
    ...
    for combination in tqdm(combinations(n_days, n_seas)):
        ...
        if check_season_len(idx, n_seas):
            centroids, error = compute_metrics(n_seas, data_to_cluster, idx)
