...
def main():
    ...
    for path, code in zip(train_paths, train_codes):
        ...
        dat = rolling_zscore_complete(dat, window_size=365*10)
    ...
    dataset_train.to_netcdf(os.path.join(os.getcwd(),'results', 'files', f'train_NOnorm_days.nc'))
    ...
    dataset_train.to_netcdf(os.path.join(os.getcwd(),'results', 'files', f'train_norm_days.nc'))
    ...
    dataset_proj.to_netcdf(os.path.join(os.getcwd(),'results', 'files', f'proj_NOnorm_days.nc'))

    # try:
    #     dataset_proj = xr.merge([xr.open_dataset(path)[code] for path, code in zip(proj_paths, proj_codes)]).mean('plev')
    # except:
    #     dataset_proj = xr.merge([xr.open_dataset(path)[code] for path, code in zip(proj_paths, proj_codes)])
    ...
    dataset_proj.to_netcdf(os.path.join(os.getcwd(),'results', 'files', f'proj_norm_days.nc'))
    ...
    # mse, r2, models, histories = train_perceptron(data, n_features, n_seas=n_seasons, epochs=100)
    ...
    # array_res  = dataset_proj.to_array().values.transpose((2, 3, 1, 0))
    ...
    # predictions_xr.to_netcdf(os.path.join(os.getcwd(),'results', 'files', f'{name}_{model_name}_projection_prediction_{scenario_model[0]}_days.nc'))